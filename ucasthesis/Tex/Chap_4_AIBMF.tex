...
本文接着比较了饿了么、美团、今日头条和网易云音乐四款应用在内容类型（content type）的取值的分布情况。计算了不同的content type取值在不同应用上的比例，得到分布情况。
\begin{figure}[!htbp]
    \centering
    \begin{subfigure}[b]{0.35\textwidth}
...
以上涉及到的超参数M和N将再本章后面进行详细分析。\usepackage{babel}
...
\begin{figure}[!htbp]
	...
	\bicaption{不同N取值表现}{Performance of Different N}
