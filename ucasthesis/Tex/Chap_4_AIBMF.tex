\chapter{HTTPS流量应用多视图特征识别方法的研究}\label{chap:AIBMF}

\section{引入}
HTTPS流量经过加密之后，传统的DPI方法已经不能够有效地完成识别任务了。而传统地机器学习方法又需要大量地特征工程来提取特征，并且这些特征很容易受到网络环境的影响而不具备鲁棒性。
...
基于以上，我们采取深度学习的方法，从不同的视图特征进行研究，使用深度学习的方法提取抽象的特征，构建识别的模型。
...
\begin{itemize}
	\item CPU: 24 Intel (R) Xeon CPU E5-2650 v4 @ 2.2GHz
	\item GPU: 2 GTX TITAN X
	\item RAM: 128GB
...
我们收集了100,000多个流数据，并研究了各种应用程序的数据包计数分布。如图4所示，超过100,000个流的数据包计数的中位数为11，一个流中的数据包计数超过16只占32％，在我们的方法中，每个流的数据包计数设置为16，第一个 每个流中获取16个数据包。
% =================================
% Fig. 03: Model architecture
% =================================
\begin{figure}[!htbp]
...
我们的研究工作还对比了机器学习的方法，首先使用特征工程的方法构建了23种特征，其中7种特征是根据HTTPS协议定义的特征，属于HTTPS流量特有的特征，其余的16种特征是流量分析工作常用的特征。
\begin{table}
