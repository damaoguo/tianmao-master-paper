...
\emph{（i）} SSL/TLS协议本质上是移动应用程序客户端和服务器之间通信的一种语言，这种语言的具体表现是数据包序列；

\emph{（ii）}因为协议状态机是对网络流的总体抽象描述，所以可以从不同的协议状态信息中构造应用程序指纹。如消息类型，内容类型等协议定义的参数信息。
...
\section{方法描述}
本文构建的识别模型如图\ref{fig:AIBMF-Architecture}所示，由预处理，特征学习，流量识别三个步骤构成。在预处理时，从原始的流样本（pcap）文件中抽取识别视图；在特征学习时，针对识别视图使用深度学习方法提取抽象特征；在应用识别时，利用抽象特征完成精细化识别任务。
\begin{figure}[!htbp]
	\centering
...
\subsection{数据流预处理}
在数据集构建部分将流量数据按照流进行提取，形成的数据集中每一条流就是一个待识别的样本。 整个流程的集合可以表示为
\begin{equation}
    D = \{{\{f_i\}}_{i=1}^{i=N}\}_{j=1}^{j=C} 
\end{equation}
这里$f_i$表示第$i$条流。本章定义的视图分从流量负载，内容类型部分和数据包大小三个部分提取。
...
如图\ref{fig:payload-eda}所示，图中横坐标表示的是ASCII编码的取值，纵轴为取值在这个应用所有的样本中所占的比例，可以发现针对不同的应用，负载（payload）的分布情况存在差异，为了更加进一步探索payload对于流量识别的作用，本文将其定义为PB视图。
%-------------------------------------------------------------------------
\subsubsection{内容类型}
...
\emph{（iii）}对于PB视图，通过将有效载荷字节值视为相应的ASCII码，对于每一个packet取前N个字节，形成一个$ M \times N $维向量$v_{pb}$，然后进一步转换为0-255（ 用-1填充）。
...
\begin{itemize}
	\item \textbf{PB视图特征学习} 
	%
	本章使用一维卷积神经网络处理 $v_{pb}$向量,如公式：\ref{eqn:1D-payload}。在处理过程中, 使用了两层卷积神经网络, 每一层卷积的卷积核数量是 64, 卷积核的大小是$1 \times 3$, 卷积操作中本章采用的是max pooling。经过处理后，原始的payload部分的转换为64维的向量 $\alpha$.
	%
	\begin{equation}
...
\begin{itemize}
    \item TP是分类为应用程序$ i $的流量确切属于应用程序$ i $的流的数量。
    \item FN是指属于应用程序$i$的流最终没有被识别为应用程序$i$的流的数量。
...
在测试阶段，\emph{Precision（P）}显示预测到应用程序$ i $的流量是实际应用程序$ i $，\emph{Recall（R）}显示预测正确的流量与属于应用程序$i$的所有流的比值，以及HTTPS流量的综合评价\emph{F1}值用作评估标准。 \emph {P}根据公式（\ref{eqn:P}）计算； \emph {R}根据公式\ref{eqn:R}计算； F1值是结合两个指标的综合评价指标，是根据公式（\ref{eqn:F1}）计算的。 为了在所有$K$个应用程序上评估本文的方法，并将平均值$ P $，$ R $，$ F_1 $ --- $ Macro \ Precision $，$ Macro \ Recall $，$ Macro \ F_1 $作为整体指标 式（\ref{eqn:macro-P}），（\ref{eqn:macro-R}），（\ref{eqn:macro-F1}）。 
...
针对前20个应用100,000多个流数据，并研究了各种应用程序的数据包计数分布。如图\ref{fig:packet-count-all}所示，超过100,000个流的数据包计数的中位数为11，一个流中的数据包计数超过16只占32％，针对每一条流，取用不同的packet的数目进行对比实验,如图\ref{fig:packet-count-acc}，发现，随着packet的数目的增加，模型的识别效果也在持续增加，当packet的数目增加为16个的时候，识别的效果不再由明显的变化，但是经过分析，随着packet数目的增加，模型所需要的空间复杂度和时间复杂度都会增加。在这里，将packet的数目取为16.
...
在实验的过程种，如图\ref{fig:performance-dl}，分别比较了仅仅使用一维卷积神经网络、二维卷积神经网络处理payload视图和AIBMF模型训练的过程，对比了在识别的过程中，loss和accuracy的变化过程。发现在训练的过程中，AIBMF模型损失下降更快，即更快收敛，准确率也领先其他的深度学习模型。
\begin{figure}[!htbp]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
...
如图\ref{fig:AIBMF-confusematrix}，针对20个移动应用1万条测试样本，绘制混淆矩阵来观察识别的效果。整体上，识别的效果非常好，其中大多数的错误是由于不同的应用之间可能共享服务所致，尤其针对那些由同一个公司开发的应用而言，识别能力存在一定的混淆。有一些应用将其API提供给了其他的应用作为公共接口，如百度地图，这给AIBMF模型的识别能力带来了一定的影响。另外在实验中，QQ软件的识别效果最差。 可能的原因是QQ使用的通信协议主要是UDP，并补充了TCP协议。第三章捕获的流量主要从Qzone捕获，该流量混合了许多其他应用程序的流量。
...
\section{本章小结}
人工定义的统计特征越复杂，流量识别任务的通用性就越差。但是，仅使用深度学习忽略协议设计的信息来处理有效负载，识别能力也受到限制。实际上，协议定义的一些有意义的信息（例如内容类型）提取代价很低，在流量分析中却很有用。因此，从多个角度分析HTTPS流量并组合不同的功能可能是将来加密流量分析中的有力选择。本章节提出了一种通过HTTPS流量进行应用程序识别的方法---AIBMF。该方法能够分析HTTPS应用流量并将HTTPS流量与特定的移动应用相关联。