\chapter{基于HTTPS流量的应用识别技术及其介绍}\label{chap:https}
...
\subsection{基于Openssl的demo}
\url{https://blog.csdn.net/sardden/article/details/42705897}
...
3. 编译方式:
gcc \-o ssl\_server ssl\_server.c \-Wall \-g \-lssl \-lcrypto
gcc \-o ssl\_client ssl\_client.c \-Wall \-g \-lssl \-lcrypto

4. 生产私钥和证书
openssl genrsa -out privkey.pem 1024
openssl req -new -x509 -key privkey.pem -out CAcert.pem -days 1095
生成的私钥是server来保存和使用的，在产生证书的时候也需要添加私钥。客户端是不需要这个私钥的。


5. 程序运行方式:
%./ssl\_server 7838 1 CAcert.pem privkey.pem
%./ssl\_client 127.0.0.1 7838

分别在两台机器上启动client和server，使用tcpdump指定了host，这样运行起来之后的结果为：



\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.80\textwidth]{openssl_demo_server_wireshark}
	\bicaption{SSL通信(握手及应用数据传输)）}{}
	\label{fig:openssl_demo_server_wireshark}
\end{figure}

在client hello阶段包含的信息较为丰富，主要关注的是extension扩展，例如重要的字段server name扩展下的SNI，对于我们进行流量识别有很大帮助。
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.60\textwidth]{openssl_demo_client_hello}
	\bicaption{client hello}{}
	\label{fig:openssl_demo_client_hello}
\end{figure}

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.80\textwidth]{openssl_demo_client}
	\bicaption{客户端完整过程 }{}
	\label{fig:openssl_demo_client}
\end{figure}


\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.80\textwidth]{openssl_demo_application_data}
	\bicaption{应用数据的传输被加密}{}
	\label{fig:openssl_demo_application_data}
\end{figure}
