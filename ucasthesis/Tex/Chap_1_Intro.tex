...
本文的目标之对加密的网络流量进行精细化分类，精细化识别结构如图\ref{fig:detail-identification}，精细化识别的识别粒度更加精细，是将单条HTTPS流和产生它的应用进行关联。
\begin{figure}[!htbp]
	\centering
...
基于DPI(深度包)的方法只能够识别明文流量，一旦服务采用了加密的方式，就会隐去载荷部分的特征，不再可见。但是HTTPS协议通信过程中依然会泄露一些信息可以被用于完成流量识别任务，当前有相当多的工作是基于HTTPS协议的识别工作。
...
针对传统的特征工程结合机器学习的识别方法存在的特征定义选择困难，依赖特征工程高，特征建模能力弱的问题以及当前很多使用深度学习的方法解决HTTPS流量精细化分类任务的作，一味追求深度学习的end-to-end方案，忽视HTTPS协议本身定义（即完全不考虑容易获取的专家知识的作用）的现状，本文从HTTPS协议本身的定义和通信过程出发，提出了一种基于多视图特征的HTTPS流量精细化分类方法，多视图特征由负载(payload)、包大小(packet size)、和HTTPS通信过程中每个packet的都会携带的标记---内容类型(content type)三个部分构成。针对三个视图特征采用深度学习的方法提取抽象特征，由三个视图提取的抽象特征共同完成识别任务。

\subsection{基于增量学习的新移动应用识别方法}
