%---------------------------------------------------------------------------%
%->> Titlepage information
%---------------------------------------------------------------------------%
%-
%-> 中文封面信息
%-
\confidential{}% 密级：只有涉密论文才填写
\schoollogo[scale=0.095]{ucas_logo}% 校徽
\title{基于HTTPS流量的移动应用识别技术研究}% 论文中文题目
\author{田茂}% 论文作者
\advisor{张永铮~研究员~中国科学院信息工程研究所\\}% 指导教师：姓名 专业技术职务 工作单位
%\advisor{指导教师一\\指导教师二\\指导教师三}% 多行指导教师示例
\degree{硕士}% 学位：学士、硕士、博士
\degreetype{工学}% 学位类别：理学、工学、工程、医学等
\major{计算机软件与理论}% 二级学科专业名称
\institute{中国科学院信息工程研究所}% 院系名称
%\institute{中国科学院力学研究所\\流固耦合实验室}% 多行院系名称示例
\date{2020~年~6~月}% 毕业日期：夏季为6月、冬季为12月
%-
%-> 英文封面信息
%-
\TITLE{Mobile Application Identification Over HTTPS Traffic}% 论文英文题目
\AUTHOR{Mao Tian}% 论文作者
\ADVISOR{Supervisor: Professor Yongzheng Zhang}% 指导教师
\DEGREE{Master}% 学位：Bachelor, Master, Doctor, Postdoctor。封面据英文学位名称自动切换，需确保拼写准确
\DEGREETYPE{Engineering}% 学位类别：Philosophy, Natural Science, Engineering, Economics, Agriculture 等
\MAJOR{Computer Software and Theory}% 二级学科专业名称
\INSTITUTE{Institute of Information Engineering, Chinese Academy of Sciences}% 院系名称
\DATE{June, 2020}% 毕业日期：夏季为June、冬季为December
%---------------------------------------------------------------------------%