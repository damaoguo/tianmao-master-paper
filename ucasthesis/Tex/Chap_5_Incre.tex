\usepackage{}\chapter{基于增量学习的HTTPS新应用识别}\label{chap:transfer}
\section{引言}
...
此阶段的目标是更新内存中的样本，并确保训练集的大小不会急剧增加。也就是说，在更新的内存中，我们将内存大小固定为$ K $个流量（本文中$ K = 100,000 $），并且与每个应用程序类别相对应的样本数为$ \ frac {100,000} {C} $，此处$ C $表示应用程序的类别。

然后，我们采用两个操作：

\emph{（i）}选择要存储的新样本：我们执行herding采样\citep{welling2009herding}，该策略根据与该类别的平均样本的距离生成一类样本的排序列表；
％
给定样本的排序列表，选择列表中的前$ n $个样本。
％
这些样本根据平均值最能代表该类别。
％
\emph{（ii）}删除剩余的样本：由于样本存储在排序列表中，因此此操作很简单。
％
存储单元仅需要从每个类别的样本集的末尾删除样本。
％
请注意，执行此操作后，删除的样本将不再使用。
...
图：\ref{fig:IncreAIBM-confuse-matrix}是IncreAIBMF模型在50个应用，2万余条样本上的识别效果。
\begin{figure}[!htbp]
	\centering
