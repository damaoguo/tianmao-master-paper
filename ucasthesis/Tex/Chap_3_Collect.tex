...
在课题研究过程中，需要大量的数据支持。然而，当前所公布的开源数据较少，数据量有限，分类的粒度较为粗糙。本章详细地描述了移动应用HTTPS流量数据地采集和处理方法，构建了一个50个不同类型的移动应用的流量数据集，规模为20万条流样本。
...
\begin{itemize}
    ...
    \item Intrusion detection evaluation dataset (ISCXIDS2012) \footnote{https://www.unb.ca/cic/datasets/ids.html}：该数据集包括标记的网络跟踪，完整数据包有效负载（pcap格式），以及相关的配置文件，供研究人员公开使用。如表\ref{tab:ISCXIDS2012}所示，该数据集由7天的网络活动（正常和恶意）组成：
     \begin{table}[!htbp]
    ...
    \item MAWILab\footnote{http://mawi.wide.ad.jp/mawi/}：\citep{mawilab}发布了MAWILab数据集，可帮助研究人员评估流量异常检测方法。它由一组用于定位MAWI存档中异常流量的标签（样本点B和F）组成。使用基于图形的高级方法获得标签，该方法比较并组合了不同且独立的异常检测器。 数据集每天更新，以包括来自即将到来的应用程序和异常的新流量。该数据集是用于评估异常流量检测方法的，不符合本文的研究场景。
...
\section{数据集构建}
\subsection{数据流抽取}
本文以流为单位进行研究和标记，在采集流量的时候，保存的同一个pcap文件是由多条流构成的。在流量被采集后，通过五元组信息---\textit{[源地址，源端口，目的地址，目的端口，传输协议]}将pcap文件进行切分，pcap文件中的包（packet）重组后形成新的pcap文件，被切分后，每一个pcap文件都是一条样本，本文中使用的抽取方法介绍如下。
...
\subsection{数据集概览}
本章基于HTTPS协议创建流量数据集，共采集了旅行交通、社交、影音视听、时尚购物新闻资讯、居家生活、聊天、图书阅读、金融理财、实用工具、游戏共计11种类型，50个应用的流量，使用sliptcap切分后得到20万多万条流量样本。
