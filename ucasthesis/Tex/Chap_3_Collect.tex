...
在基于HTTPS流量的移动应用识别技术研究中，需要大量有标记的数据的支持，所谓的标记就是产生流量的应用的应用名称，而不是使用的协议类别等标签。然而，当前所公开的开源数据较少，数据量有限，流量数据标记较为粗糙，不满足本文的研究工作的需要。本章详细地描述了移动应用HTTPS流量数据的采集和处理方法，构建了一个覆盖11种应用类别、涉及50个移动应用、共计约20多万条HTTPS加密网络数据流的有标记数据集。

\section{引言}

本章首先调研了当前已经公开的流量数据集，总结这些公开数据集的来源、规模、协议、用途以及获取方式。现有的公开并被多项研究工作使用的数据集如下：
\begin{itemize}
    ...
    \item NLANR AMP Data\footnote{https://labs.ripe.net/datarepository/data-sets/nlanr-amp-data}：此数据是NLANR研究小组收集的一组测量数据（包括ping /traceroute）。 数据由多达130个有利点的网格中的测量组成，并且这些测量在1998年至2006年之间进行。该数据可用于Internet的纵向研究。RIPE数据存储库中的可用数据是NLANR网站上可用的原始数据的重新制作版本。该数据集包括的是\emph{ping/traceroute}和相关的流量，不符合本文的研究场景所需。
    ...
    \item Waikato Internet Traffic Storage(WITS)\footnote{https://wand.net.nz/wits/}：该数据集目前有33个不同的集合构成，数据规模较为庞大，且数据来源较为丰富，但是该数据集采集的是混合流量，样本缺少精细化的标记，因此难于用于训练精细化的流量识别方法，无法满足本文中的精细化识别场景的需求。
    
    
\end{itemize}


本文的研究目的在于通过对大量的应用的HTTPS流量数据进行分析，得到一种可靠准确的分析方法，经过分析，以上提到的公开数据集在通信协议、数据规模、数据标签、数据格式或者使用场景等不满足本文针对HTTPS流量进行精细化识别的要求，因此需要构建新的数据集来支撑本文的研究工作。

\section{方法阐述}
在本文章中介绍一种自动化移动应用HTTPS流量标记方法，图\ref{fig:data-collect}给出了本章所提出的自动化移动应用HTTPS 流量标记方法的框架图，主要由APP 爬虫模块、应用调度模块、App 运行环境模块、
事件注入模块和守护进程模块组成，详细介绍如下。


\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.80\textwidth]{Data-collect.pdf}
	\bicaption{数据采集标记架构。}{Data collection and labelling architecture.}
...
本文按流进行研究和标记，在采集流量的时候，保存的同一个pcap文件是由多条流构成的。在流量被采集后，通过五元组信息---\textit{[源地址，源端口，目的地址，目的端口，传输协议]}将pcap文件进行切分，pcap文件中的数据包重组后形成新的pcap文件，被切分后，每一个pcap文件都是一条样本，本文中使用的抽取方法介绍如下。
...
\section{巨型移动应用流量标记}
上述提出的自动化移动应用HTTPS流量标记方法在巨型移动应用的标注任务上存在一定不足，例如在今日头条集成了西瓜视频，在采集数据标注的时候本身会出现矛盾，即今日头条中关于西瓜视频的流，会被标注为今日头条。该问题是由当前的移动应用生态并非完全隔离且对流量的标记的认定规则存在一定的模糊性导致的，这种标记冲突的影响会随各个应用独立的样本容量规模增加而变得微小。以今日头条和西瓜视频为例，在今日头条中混合的西瓜视频的流量相比于大规模标记的西瓜视频流量占比非常小，本文在第四章、第五章提出的识别方法允许这种小量的错误标记样本存在。

\section{本章小结}
本章提出了一种自动化的移动应用HTTPS流量采集标记方法，并实现了相关的系统，主要关键功能包括应用的爬取、应用的自动安装、应用的自动驱动执行、流量捕获与标记等功能。通过本章的研究，为本文后续的研究工作提供了可靠的有标记的实验数据集。